...
def plot_input_control(data):
    ...
    axs2[0].plot(data[constants.ANTI_SURGE_VALVE_POSITION + '|average'], label='_nolegend_')
    axs2[0].set_title('Anti-surge valve position %')

    axs2[1].plot(data[constants.SUCTION_THROTTLE_VALVE_POSITION + '|average'], label='_nolegend_')
    axs2[1].set_title('Suction throttle valve position %')

    axs2[2].plot(data[constants.COMPRESSOR_SHAFT_POWER + '|average'], label='_nolegend_')
